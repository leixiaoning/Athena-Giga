...
class MtlTransformerCtc(BaseModel):
    ...
    def __init__(self, data_descriptions, config=None):
        ...
        # for WFST
        self.vocab = {}
        m = data_descriptions.hparams.text_config["model"]
        sp = text.SentencepieceTokenizer(model=open(m, "rb").read())
    ...
    def call(self, samples, training=None):
        ...
        attention_logits, encoder_output = self.model(samples, training=training)#(4, 17, 5001), (4, 51, 512)
        ctc_logits = self.decoder(encoder_output, training=training)#(4, 51, 5001])
